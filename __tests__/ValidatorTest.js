...
describe('InputValidator에 대한 Test', () => {
  ...
  describe('menuCountInput', () => {
    it('유효한 주문에 대해 에러를 던지지 않고 값을 반환한다.', () => {
      ...
      expect(() => testMenuCountInput(validOrder)).not.toThrow();
    ...
    it('메뉴에 없는 메뉴 주문 시 에러를 던져야 한다.', () => {
      ...
      expect(() => testMenuCountInput(invalidOrder)).toThrow(ERROR_MESSAGE.invalidOrder);
    });
    ...
    it('중복된 메뉴에 대해 에러를 던져야 한다.', () => {
      ...
      expect(() => testMenuCountInput(duplicateOrder)).toThrow(ERROR_MESSAGE.invalidOrder);
    });
